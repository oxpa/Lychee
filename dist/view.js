...
var _templateObject = _taggedTemplateLiteral(["<svg class='iconic $", "'><use xlink:href='#$", "' /></svg>"], ["<svg class='iconic $", "'><use xlink:href='#$", "' /></svg>"]),
    _templateObject2 = _taggedTemplateLiteral(["<div class='divider' id='$", "'><h1>$", "</h1></div>"], ["<div class='divider' id='$", "'><h1>$", "</h1></div>"]),
...
build.divider = function (title) {
	...
	html += lychee.html(_templateObject2, id, title);
...
header.bind = function () {
	...
	header.dom('.button_add').on(eventName, function (e) {
		contextMenu.add(album.getID(), e);
	});
...
header.setMode = function (mode) {
	...
	switch (mode) {
		...
		case 'album':
			...
			if (album.json.content === false && album.subjson.num == 0) $('#button_archive').hide();else $('#button_archive').show();
